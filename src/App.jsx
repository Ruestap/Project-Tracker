...
const TIPO_META = {
  "tarea":      {icon:"📋",label:"Tarea",     color:"#00b5b4"},
  "recurrente": {icon:"🔄",label:"Recurrente",color:"#8b5cf6"},
};
const avBg  = n=>{const h=(n||"?").split("").reduce((a,c)=>a+c.charCodeAt(0),0);return["#00b5b4","#8b5cf6","#ec4899","#14b8a6","#f97316","#0ea5e9"][h%6];};
...
function CatalogRow({value,onSave,onDelete,accent}) {
  ...
  return(
    <div style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",borderRadius:8,background:"#f8fafc",border:`1px solid ${accent}22`,marginBottom:6}}>
      {ed?<>
        <input autoFocus value={v} onChange={e=>setV(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")commit();if(e.key==="Escape"){setV(value);setEd(false);}}}
          style={{flex:1,background:"rgba(8,14,26,.9)",border:`1px solid ${accent}55`,borderRadius:6,padding:"5px 10px",color:"#1a2f4a",fontSize:12,outline:"none"}}/>
        ...
        <button onClick={()=>{setV(value);setEd(false);}} style={{padding:"4px 10px",borderRadius:6,border:"1px solid #c8d8e8",background:"transparent",color:"#64748b",cursor:"pointer",fontSize:11}}>✕</button>
      </>:<>
  ...
  return(
    <div style={{display:"flex",gap:8,marginTop:10}}>
      <input value={v} onChange={e=>setV(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()} placeholder={placeholder}
        style={{flex:1,background:"rgba(8,14,26,.9)",border:`1px dashed ${accent}44`,borderRadius:8,padding:"8px 12px",color:"#1a2f4a",fontSize:12,outline:"none"}}/>
...
function LoginScreen({adminPin,visitorPin,onLogin}){
  ...
  return(
    <div style={{fontFamily:"'DM Sans',sans-serif",background:"#f0f4f8",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      ...
      <div style={{width:380,background:"#ffffff",borderRadius:20,padding:40,border:"1px solid #c8d8e8",boxShadow:"0 20px 60px rgba(0,0,0,.12)",textAlign:"center"}}>
        <div style={{width:70,height:70,borderRadius:20,background:"linear-gradient(135deg,#00b5b4,#1a2f4a)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:34,margin:"0 auto 24px",boxShadow:"0 8px 30px rgba(0,181,180,.3)"}}>📊</div>
        <h1 style={{fontFamily:"'Syne',sans-serif",fontSize:24,fontWeight:800,margin:"0 0 8px",color:"#1a2f4a",letterSpacing:"-.01em"}}>PROJECT TRACKER</h1>
        {step==="pin"?<>
          <p style={{margin:"0 0 28px",fontSize:12,color:"#5a7a9a",lineHeight:1.7}}>Ingresa tu código de acceso</p>
          <div style={{animation:shake?"shake .4s ease":"none",marginBottom:16}}>
            <input autoFocus type="password" value={pin} onChange={e=>setPin(e.target.value)} onKeyDown={e=>e.key==="Enter"&&tryPin()} placeholder="••••••••"
              style={{width:"100%",padding:"14px",borderRadius:12,border:`2px solid ${err?"#ef4444":"#c8d8e8"}`,background:"#f8fafc",color:"#1a2f4a",fontSize:18,outline:"none",textAlign:"center",letterSpacing:8,boxSizing:"border-box",marginBottom:12}}/>
            <button onClick={tryPin} style={{width:"100%",padding:"13px",borderRadius:12,border:"none",background:"#00b5b4",color:"white",cursor:"pointer",fontSize:14,fontWeight:700}}>
              {err?"❌  Código incorrecto":"Ingresar →"}
            </button>
          </div>
          <div style={{padding:"12px 16px",borderRadius:10,background:"#f0f4f8",border:"1px solid #c8d8e8",fontSize:10,color:"#5a7a9a",lineHeight:1.9}}>
            🔐 Admin — control total &nbsp;|&nbsp; 👁️ Visitante — agregar y visualizar
          </div>
        </>:<>
          <p style={{margin:"0 0 24px",fontSize:12,color:"#2dd4bf",lineHeight:1.7}}>✅ Acceso verificado<br/><span style={{color:"#5a7a9a"}}>¿Cómo aparecerás en el tablero?</span></p>
          <input autoFocus value={name} onChange={e=>setName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&name.trim()&&onLogin("visitor",name.trim())} placeholder="Tu nombre completo"
            style={{width:"100%",padding:"13px",borderRadius:12,border:"2px solid #00b5b4",background:"#f8fafc",color:"#1a2f4a",fontSize:14,outline:"none",textAlign:"center",boxSizing:"border-box",marginBottom:12}}/>
          <button onClick={()=>name.trim()&&onLogin("visitor",name.trim())} disabled={!name.trim()}
            style={{width:"100%",padding:"13px",borderRadius:12,border:"none",background:name.trim()?"#00b5b4":"#1a2f4a",color:name.trim()?"white":"#94a3b8",cursor:name.trim()?"pointer":"not-allowed",fontSize:14,fontWeight:700}}>
            Entrar al tablero →
          </button>
        </>}
      </div>
...
function TaskModal({task,isAdmin,areas,resps,userName,onSave,onClose}){
  ...
  const lbl={fontSize:10,color:"#00b5b4",fontWeight:700,letterSpacing:".06em",display:"block",marginBottom:4};
  const save=()=>{
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(26,47,74,.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:50,backdropFilter:"blur(4px)"}}>
      <div style={{background:"#ffffff",borderRadius:16,padding:28,width:"92%",maxWidth:560,border:"1px solid #c8d8e8",boxShadow:"0 20px 60px rgba(0,0,0,.15)",maxHeight:"90vh",overflowY:"auto"}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
          <div style={{width:34,height:34,borderRadius:9,background:"linear-gradient(135deg,#00b5b4,#1a2f4a)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:17}}>{editing?"✏️":"➕"}</div>
          <div>
            <h3 style={{fontFamily:"'Syne',sans-serif",fontSize:15,fontWeight:800,margin:0,color:"#00b5b4"}}>{editing?"Editar Actividad":"Nueva Actividad"}</h3>
            {!isAdmin&&<p style={{margin:0,fontSize:10,color:"#5a7a9a"}}>Las fechas solo pueden ser modificadas por el administrador</p>}
          </div>
        </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
            {Object.entries(TIPO_META).map(([k,m])=>(
              <button key={k} onClick={()=>setF(p=>({...p,tipo:k}))}
                style={{padding:"10px 8px",borderRadius:10,border:`2px solid ${f.tipo===k?m.color:m.color+"30"}`,background:f.tipo===k?m.color+"18":"rgba(0,0,0,.02)",cursor:"pointer",transition:"all .15s",textAlign:"center"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
          ...
          <div>
            <label style={{...lbl,color:isAdmin?"#00b5b4":"#5a7a9a"}}>FECHA ENTREGA *{!isAdmin&&editing?" 🔒":""}</label>
            {(isAdmin||!editing)
            ...
            <div style={{display:"flex",gap:8}}>
              {["diaria","semanal","quincenal","mensual"].map(fr=>(
                <button key={fr} onClick={()=>setF(p=>({...p,frecuencia:fr}))}
                  style={{flex:1,padding:"8px 4px",borderRadius:8,border:`2px solid ${f.frecuencia===fr?"#6c5ce7":"#c8d8e8"}`,background:f.frecuencia===fr?"#ede9fe":"#ffffff",cursor:"pointer",color:f.frecuencia===fr?"#6c5ce7":"#64748b",fontSize:11,fontWeight:700,textTransform:"capitalize"}}>
                  {fr}
                </button>
              ))}
            </div>
        ...
        {err&&<div style={{marginBottom:12,padding:"8px 12px",borderRadius:8,background:"#fee2e2",border:"1px solid #fca5a5",fontSize:11,color:"#dc2626"}}>{err}</div>}
        <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
          <button onClick={onClose} style={{padding:"8px 20px",borderRadius:8,border:"1px solid #c8d8e8",background:"transparent",color:"#64748b",cursor:"pointer",fontWeight:600,fontSize:12}}>Cancelar</button>
          <button onClick={save} style={{padding:"8px 22px",borderRadius:8,border:"none",background:"#00b5b4",color:"white",cursor:"pointer",fontWeight:700,fontSize:12}}>{editing?"Guardar Cambios":"Agregar"}</button>
        </div>
      </div>
    </div>
  );
...
function PinModal({adminPin,visitorPin,onSave,onClose}){
  ...
  const inp={width:"100%",padding:"9px 12px",borderRadius:8,border:"1px solid #c8d8e8",background:"#f8fafc",color:"#1a2f4a",fontSize:13,outline:"none",boxSizing:"border-box",letterSpacing:4,fontFamily:"monospace"};
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(26,47,74,.65)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:60,backdropFilter:"blur(8px)"}}>
      <div style={{background:"#ffffff",borderRadius:16,padding:28,width:"90%",maxWidth:400,border:"1px solid #c8d8e8"}}>
        <h3 style={{fontFamily:"'Syne',sans-serif",fontSize:15,fontWeight:800,margin:"0 0 18px",color:"#00b5b4"}}>🔑 Gestionar Códigos de Acceso</h3>
        <div style={{marginBottom:12}}><label style={{fontSize:10,color:"#00b5b4",fontWeight:700,letterSpacing:".06em",display:"block",marginBottom:4}}>🔐 CÓDIGO ADMINISTRADOR</label><input type={show?"text":"password"} value={ap} onChange={e=>setAp(e.target.value)} style={inp}/></div>
        ...
        {err&&<div style={{marginBottom:12,padding:"8px 12px",borderRadius:8,background:"#fee2e2",border:"1px solid #fca5a5",fontSize:11,color:"#dc2626"}}>{err}</div>}
        <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
          <button onClick={onClose} style={{padding:"8px 18px",borderRadius:8,border:"1px solid #c8d8e8",background:"transparent",color:"#64748b",cursor:"pointer",fontSize:12}}>Cancelar</button>
          <button onClick={save} style={{padding:"8px 20px",borderRadius:8,border:"none",background:"#00b5b4",color:"white",cursor:"pointer",fontWeight:700,fontSize:12}}>Guardar</button>
        </div>
      </div>
    </div>
  );
...
function VistaEjecutiva({tasks}){
  ...
  const cardS={background:"#ffffff",borderRadius:12,padding:"18px 20px",border:"1px solid #e2e8f0",boxShadow:"0 2px 8px rgba(0,0,0,.06)"};
  return(
    <div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr 1fr",gap:12,marginBottom:16}}>
        {[[tot,"TOTAL","#00b5b4","rgba(0,181,180,.3)"],[term,"TERMINADAS","#4ade80","rgba(34,197,94,.3)"],[proc,"EN PROCESO","#fbbf24","rgba(251,191,36,.3)"],[pend,"PENDIENTES","#94a3b8","rgba(100,116,139,.3)"],[ret,"CON RETRASO","#f87171","rgba(239,68,68,.3)"]].map(([n,l,c,bg])=>(
          <div key={l} style={{...cardS,textAlign:"center",border:`1px solid ${bg}`,background:bg.replace(".3",".06")}}>
            ...
            <div style={{fontSize:9,color:"#5a7a9a",marginTop:4,letterSpacing:".08em",fontWeight:700}}>{l}</div>
          </div>
        ))}
      </div>
      <div style={{...cardS,marginBottom:16,display:"flex",alignItems:"center",gap:20}}>
        <div style={{flex:1}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
            ...
            <span style={{fontSize:22,fontWeight:800,color:"#00b5b4",fontFamily:"'Syne',sans-serif"}}>{pct}%</span>
          </div>
          <div style={{height:12,background:"#1a2f4a",borderRadius:6,overflow:"hidden"}}>
            <div style={{width:`${pct}%`,height:"100%",background:"linear-gradient(90deg,#00b5b4,#1a2f4a)",borderRadius:6,transition:"width .5s"}}/>
          </div>
        ...
        <div style={{width:1,height:60,background:"#1a2f4a",flexShrink:0}}/>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
        <div style={cardS}>
          ...
          {proximos.length===0?<p style={{fontSize:11,color:"#5a7a9a",textAlign:"center",padding:"12px 0"}}>Sin vencimientos próximos ✓</p>:
          proximos.map(t=>{const dr=diasRestantes(t);return(
            <div key={t.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",borderRadius:8,background:"rgba(0,0,0,.02)",marginBottom:6,border:"1px solid #f0f4f8"}}>
              <Pill task={t}/>
              <span style={{flex:1,fontSize:11,fontWeight:600,color:"#1a2f4a",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={t.actividad}>{t.actividad}</span>
        ...
        <div style={cardS}>
          ...
          {atRisk.length===0?<p style={{fontSize:11,color:"#5a7a9a",textAlign:"center",padding:"12px 0"}}>Sin tareas atrasadas ✓</p>:
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        <div style={cardS}>
          ...
          {recurrentes.length===0?<p style={{fontSize:11,color:"#5a7a9a",textAlign:"center",padding:"12px 0"}}>Sin recurrentes activos</p>:
          recurrentes.map(t=>(<div key={t.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",borderRadius:8,background:"rgba(139,92,246,.04)",marginBottom:6,border:"1px solid rgba(139,92,246,.12)"}}>
            <div>
              <div style={{fontSize:11,fontWeight:600,color:"#1a2f4a",maxWidth:160,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={t.actividad}>{t.actividad}</div>
        ...
        <div style={cardS}>
          <div style={{fontSize:11,fontWeight:700,color:"#94a3b8",letterSpacing:".06em",marginBottom:12,display:"flex",alignItems:"center",gap:6}}>
            ...
            <span style={{marginLeft:"auto",padding:"2px 8px",borderRadius:10,background:"rgba(0,181,180,.15)",color:"#00b5b4",fontSize:10,fontWeight:700}}>{proximos.length}</span>
          </div>
          {proximos.length===0?<p style={{fontSize:11,color:"#5a7a9a",textAlign:"center",padding:"12px 0"}}>Sin vencimientos próximos ✓</p>:
          proximos.map(t=>{const dr=diasRestantes(t);return(
            <div key={t.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",borderRadius:8,background:"rgba(0,181,180,.04)",marginBottom:6,border:"1px solid rgba(99,102,241,.12)"}}>
              <div style={{flex:1}}>
                <div style={{fontSize:11,fontWeight:600,color:"#00b5b4",maxWidth:150,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={t.actividad}>{t.actividad}</div>
                <div style={{fontSize:10,color:"#64748b",marginTop:2}}>{t.responsable} · {fmtDate(t.fechaEntrega)}</div>
              </div>
              <span style={{fontSize:11,fontWeight:700,color:dr<0?"#ef4444":dr<=3?"#f97316":"#00b5b4",fontFamily:"monospace",flexShrink:0}}>{dr<0?`+${Math.abs(dr)}d`:dr===0?"Hoy":`${dr}d`}</span>
            </div>
          );})}
        </div>
      </div>
...
export default function App(){
  ...
  const S={
    wrap:{fontFamily:"'DM Sans',sans-serif",background:"#f0f4f8",minHeight:"100vh",color:"#1a2f4a"},
    hdr: {background:"#1a2f4a",borderBottom:"1px solid rgba(0,0,0,.12)",padding:"16px 24px 0"},
    th:  {padding:"10px 12px",textAlign:"left",color:"#5a7a9a",fontWeight:700,fontSize:10,letterSpacing:".07em",whiteSpace:"nowrap",borderBottom:"1px solid #e2e8f0",background:"#f8fafc"},
    td:  {padding:"10px 12px",borderBottom:"1px solid #e9eef5",fontSize:11,verticalAlign:"middle"},
    tabB:a=>({padding:"8px 18px",borderRadius:"8px 8px 0 0",border:"none",cursor:"pointer",fontSize:11,fontWeight:700,letterSpacing:".03em",transition:"all .15s",background:a?"#00b5b4":"transparent",color:a?"white":"#8aaabb"}),
    inp: {width:"100%",padding:"8px 11px",borderRadius:8,border:"1px solid #c8d8e8",background:"#ffffff",color:"#1a2f4a",fontSize:12,outline:"none",boxSizing:"border-box"},
  };
  ...
  if(loading) return(
    <div style={{...S.wrap,display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh"}}>
      ...
      <div style={{textAlign:"center"}}>
        ...
        <div style={{fontFamily:"'Syne',sans-serif",fontSize:16,color:"#00b5b4",fontWeight:800}}>Cargando tablero...</div>
        <div style={{fontSize:11,color:"#5a7a9a",marginTop:8}}>Conectando con Firebase</div>
      </div>
    </div>
  );
    ...
    <div style={S.wrap}>
      ...
      <div style={S.hdr}>
        <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:14,flexWrap:"wrap"}}>
          <div style={{width:36,height:36,borderRadius:10,background:"linear-gradient(135deg,#00b5b4,#1a2f4a)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>📊</div>
          <div>
            <div style={{fontFamily:"'Syne',sans-serif",fontSize:18,fontWeight:800,color:"#1a2f4a",letterSpacing:"-.01em"}}>PROJECT TRACKER</div>
            <div style={{fontSize:10,color:"rgba(255,255,255,.6)",letterSpacing:".05em"}}>{TODAY.toLocaleDateString("es-ES",{weekday:"long",day:"2-digit",month:"long",year:"numeric"}).toUpperCase()} · <span style={{color:isAdmin?"#00b5b4":"#2dd4bf",fontWeight:700}}>{isAdmin?"🔐 "+userName:"👁️ "+userName}</span></div>
          </div>
          <div style={{marginLeft:"auto",display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
            ...
            {isAdmin&&<button onClick={()=>setPinModal(true)} style={{padding:"6px 12px",borderRadius:7,border:"1px solid rgba(99,102,241,.3)",background:"rgba(0,181,180,.08)",color:"#00b5b4",cursor:"pointer",fontWeight:600,fontSize:10}}>🔑 Códigos</button>}
            <button onClick={()=>{setRole(null);setUserName("");}} style={{padding:"6px 12px",borderRadius:7,border:"1px solid #d8e4f0",background:"rgba(0,0,0,.02)",color:"#5a7a9a",cursor:"pointer",fontSize:10}}>↩ Salir</button>
          </div>
        </div>
      ...
      <div style={{padding:20}}>
            ...
            <div style={{display:"flex",gap:4}}>
              {[["todos","Todos"],["tarea","📋 Tareas"],["recurrente","🔄 Recurrentes"]].map(([v,l])=>(
                <button key={v} onClick={()=>setFilterTipo(v)}
                  style={{padding:"5px 12px",borderRadius:8,border:`1px solid ${filterTipo===v?"#00b5b4":"#c8d8e8"}`,background:filterTipo===v?"#e0fafa":"#ffffff",color:filterTipo===v?"#00b5b4":"#5a7a9a",cursor:"pointer",fontSize:10,fontWeight:600}}>
                  {l}
                </button>
              ))}
            </div>
          ...
          <div style={{background:"#ffffff",borderRadius:12,border:"1px solid #e2e8f0",overflow:"hidden",boxShadow:"0 2px 8px rgba(0,0,0,.06)"}}>
            <div style={{overflowX:"auto"}}>
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
                ...
                <tbody>
                  {filteredTasks.length===0&&<tr><td colSpan={10} style={{textAlign:"center",padding:50,color:"#cbd5e1",fontSize:12}}>Sin actividades</td></tr>}
                  {filteredTasks.map(t=>{
                    ...
                    return(
                      <tr key={t.id} onMouseEnter={e=>e.currentTarget.style.background="#f8fafc"} onMouseLeave={e=>e.currentTarget.style.background="transparent"} style={{transition:"background .1s"}}>
                        <td style={S.td}><Pill task={t}/></td>
                        <td style={{...S.td,fontWeight:600,maxWidth:200,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",color:"#1a2f4a"}} title={t.actividad}>{t.actividad}</td>
                        <td style={{...S.td,color:"#5a7a9a"}}>{t.area||"—"}</td>
                        ...
                        <td style={{...S.td,fontFamily:"monospace",color:"#5a7a9a",whiteSpace:"nowrap"}}>{fmtDate(t.fechaEntrega)}</td>
                        ...
                        <td style={S.td}><div style={{display:"flex",flexDirection:"column",gap:3}}>
                          <div style={{display:"flex",justifyContent:"space-between",gap:6}}>
                            <span style={{color:"#5a7a9a",fontFamily:"monospace",fontSize:10}}>{tiem.label}</span>
                            <span style={{color:barC,fontWeight:700,fontSize:10}}>{tiem.pct}%</span>
                          </div>
                          <div style={{width:80,height:4,background:"#1a2f4a",borderRadius:2}}><div style={{width:`${Math.min(100,tiem.pct)}%`,height:"100%",borderRadius:2,background:barC}}/></div>
                        </div></td>
                        ...
                        {isAdmin&&<td style={S.td}><div style={{display:"flex",gap:4}}>
                          <button onClick={()=>setModal({...t})} style={{padding:"4px 8px",borderRadius:5,border:"1px solid rgba(99,102,241,.3)",background:"rgba(0,181,180,.08)",color:"#00b5b4",cursor:"pointer",fontSize:10}}>✏️</button>
                          {t.tipo==="recurrente"&&t.estatus!=="terminado"&&
                            <button onClick={()=>markDone(t.id)} title="Marcar terminado y generar siguiente" style={{padding:"4px 8px",borderRadius:5,border:"1px solid #bbf7d0",background:"#f0fdf4",color:"#16a34a",cursor:"pointer",fontSize:10}}>✓🔄</button>}
                          <button onClick={()=>setDelCfm(t.id)} style={{padding:"4px 8px",borderRadius:5,border:"1px solid #fecaca",background:"#fff1f2",color:"#dc2626",cursor:"pointer",fontSize:10}}>🗑️</button>
                        </div></td>}
                      </tr>
                    );
                  })}
        ...
        {tab===2&&<div>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
            <button onClick={()=>navM(-1)} style={{padding:"6px 14px",borderRadius:7,border:"1px solid #c8d8e8",background:"#ffffff",color:"#1a2f4a",cursor:"pointer",fontWeight:600,fontSize:11}}>← Anterior</button>
            <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:15,color:"#1a2f4a",minWidth:170,textAlign:"center"}}>{MONTHS[ganttM].toUpperCase()} {ganttY}</div>
            <button onClick={()=>navM(1)} style={{padding:"6px 14px",borderRadius:7,border:"1px solid #c8d8e8",background:"#ffffff",color:"#1a2f4a",cursor:"pointer",fontWeight:600,fontSize:11}}>Siguiente →</button>
          </div>
          <div style={{background:"#ffffff",borderRadius:12,border:"1px solid #e2e8f0",overflow:"hidden",boxShadow:"0 2px 8px rgba(0,0,0,.06)"}}>
            <div style={{overflowX:"auto"}}>
              <table style={{borderCollapse:"collapse",fontSize:10,minWidth:"max-content"}}>
                <thead><tr>
                  {[["TIPO",60],["ACTIVIDAD",180],["RESPONSABLE",120],["ESTATUS",90],["ALERTA",100],["F.ENTREGA",85]].map(([h,w])=>(
                    <th key={h} style={{...S.th,minWidth:w,maxWidth:w,borderRight:"1px solid #f8fafc"}}>{h}</th>
                  ))}
                  {monthDays.map(d=>{const isT=d===TODAY.getDate()&&ganttM===TODAY.getMonth()&&ganttY===TODAY.getFullYear(),wd=new Date(ganttY,ganttM,d).getDay(),wk=wd===0||wd===6;
                    return <th key={d} style={{...S.th,width:24,minWidth:24,textAlign:"center",padding:"10px 1px",color:isT?"#fbbf24":wk?"#cbd5e1":"#94a3b8",background:isT?"#fffbeb":wk?"#f0f4f8":"#ffffff",borderRight:"1px solid rgba(0,0,0,.02)"}}>
                ...
                <tbody>
                  {filteredTasks.map(t=>{const prog=getProgreso(t),gc=ganttColor(t),m=TIPO_META[t.tipo]||TIPO_META.tarea;
                    ...
                    return <tr key={t.id} onMouseEnter={e=>e.currentTarget.style.background="#f8fafc"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                      <td style={{...S.td,borderRight:"1px solid #f8fafc",textAlign:"center"}}><span style={{fontSize:14}}>{m.icon}</span></td>
                      <td style={{...S.td,fontWeight:600,color:"#1a2f4a",borderRight:"1px solid #f8fafc",maxWidth:180,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={t.actividad}>{t.actividad}</td>
                      <td style={{...S.td,color:"#5a7a9a",borderRight:"1px solid #f8fafc",whiteSpace:"nowrap"}}><div style={{display:"flex",alignItems:"center",gap:5}}>
                      ...
                      <td style={{...S.td,borderRight:"1px solid #f8fafc"}}><Badge c={ESTAT_P[t.estatus].c} b={ESTAT_P[t.estatus].b} br={ESTAT_P[t.estatus].br} size={9}>{t.estatus.toUpperCase()}</Badge></td>
                      <td style={{...S.td,borderRight:"1px solid #f8fafc"}}><Badge c={PROG_P[prog].c} b={PROG_P[prog].b} br={PROG_P[prog].br} size={9}>{prog}</Badge></td>
                      <td style={{...S.td,color:"#5a7a9a",fontFamily:"monospace",whiteSpace:"nowrap",borderRight:"1px solid #f8fafc",fontSize:10}}>{fmtDate(t.fechaEntrega)}</td>
                      {monthDays.map(d=>{const ir=inRange(t,d),wd=new Date(ganttY,ganttM,d).getDay(),wk=wd===0||wd===6,isT=d===TODAY.getDate()&&ganttM===TODAY.getMonth()&&ganttY===TODAY.getFullYear();
                        return <td key={d} style={{padding:"8px 1px",textAlign:"center",borderRight:"1px solid rgba(0,0,0,.02)",background:isT?"rgba(251,191,36,.04)":wk?"rgba(0,0,0,.18)":undefined}}>
                          {ir&&barColor&&<div style={{width:18,height:12,borderRadius:2,margin:"auto",background:barColor,opacity:.85}}/>}
                        </td>;})}
                    </tr>;})}
                  {filteredTasks.length===0&&<tr><td colSpan={6+daysInMonth} style={{textAlign:"center",padding:40,color:"#cbd5e1"}}>Sin actividades</td></tr>}
                </tbody>
              </table>
            </div>
            <div style={{padding:"10px 18px",borderTop:"1px solid #e2e8f0",display:"flex",gap:18,flexWrap:"wrap"}}>
              {[["#22c55e","Terminado"],["#eab308","En Proceso"],["#ef4444","Con Retraso"],["#8b5cf6","Recurrente"]].map(([c,l])=>(
                <div key={l} style={{display:"flex",alignItems:"center",gap:5,fontSize:10,color:"#5a7a9a"}}><div style={{width:12,height:8,borderRadius:2,background:c}}/>{l}</div>
              ))}
            </div>
          </div>
        </div>}
        ...
        {tab===3&&isAdmin&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
          <div style={{background:"#ffffff",borderRadius:12,padding:22,border:"1px solid #e2e8f0",boxShadow:"0 2px 8px rgba(0,0,0,.06)"}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
              <div style={{width:34,height:34,borderRadius:9,background:"linear-gradient(135deg,#00b5b4,#1a2f4a)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:17}}>🏢</div>
              <div><h3 style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:13,color:"#00b5b4",margin:0}}>ÁREAS</h3><p style={{margin:0,fontSize:10,color:"#5a7a9a"}}>{areas.length} registradas</p></div>
            </div>
            <div style={{maxHeight:280,overflowY:"auto",paddingRight:4}}>
              {areas.map((a,i)=><CatalogRow key={`${i}-${a}`} value={a} accent="#00b5b4" onSave={v=>updateArea(i,v)} onDelete={()=>deleteArea(i)}/>)}
            </div>
            <AddRow placeholder="Nueva área…" onAdd={addArea} accent="#00b5b4"/>
          </div>
          <div style={{background:"#ffffff",borderRadius:12,padding:22,border:"1px solid #e2e8f0",boxShadow:"0 2px 8px rgba(0,0,0,.06)"}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
              ...
              <div><h3 style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:13,color:"#2dd4bf",margin:0}}>RESPONSABLES</h3><p style={{margin:0,fontSize:10,color:"#5a7a9a"}}>{resps.length} registradas</p></div>
            </div>
            <div style={{maxHeight:280,overflowY:"auto",paddingRight:4}}>
              {resps.map((r,i)=>(
                <div key={`${i}-${r}`} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",borderRadius:8,background:"#f8fafc",border:"1px solid #e2e8f0",marginBottom:6}}>
      ...
      {delCfm!==null&&<div style={{position:"fixed",inset:0,background:"rgba(26,47,74,.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:50,backdropFilter:"blur(4px)"}}>
        <div style={{background:"#ffffff",borderRadius:14,padding:28,maxWidth:340,width:"90%",border:"1px solid #fca5a5",textAlign:"center"}}>
          <div style={{fontSize:32,marginBottom:10}}>⚠️</div>
          <h3 style={{fontFamily:"'Syne',sans-serif",color:"#f87171",fontWeight:800,margin:"0 0 8px"}}>¿Eliminar actividad?</h3>
          <p style={{color:"#5a7a9a",fontSize:12,margin:"0 0 20px"}}>Esta acción no se puede deshacer.</p>
          <div style={{display:"flex",gap:10,justifyContent:"center"}}>
            <button onClick={()=>setDelCfm(null)} style={{padding:"8px 18px",borderRadius:8,border:"1px solid #c8d8e8",background:"transparent",color:"#64748b",cursor:"pointer",fontSize:12}}>Cancelar</button>
